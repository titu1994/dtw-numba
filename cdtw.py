...
#     def parallel_wrapper(*argv, **kwargs):
...
#         result = executor.submit(parallel_wrapper, *argv, **kwargs)
#
...
def dtw_distance(series1, series2, distance_mat):
    l1 = series1.shape[1]
    l2 = series2.shape[1]

    di, dj = cuda.grid(2)

    if di >= distance_mat.shape[0] or dj >= distance_mat.shape[1]:
        return

    if di >= series1.shape[0] or dj >= series2.shape[0]:
        return

    E = cuda.local.array((T1, T2), float64)

    # Fill First Cell
    v = series1[di][0] - series2[dj][0]
    E[0][0] = (v * v).real

    # Fill First Column
    for i in range(1, l1):
        v = series1[di][i] - series2[dj][0]
        E[i][0] = E[i - 1][0] + (v * v).real

    # Fill First Row
    for i in range(1, l2):
        v = series1[di][0] - series2[dj][i]
        E[0][i] = E[0][i - 1] + (v * v).real

    for i in range(1, l1):
        for j in range(1, l2):
            v = series1[di][i] - series2[dj][j]
            v = (v * v).real
    ...
    v = cmath.sqrt(E[T1 - 1][T2 - 1])

    distance_mat[di][dj] = (v).real
...
class KnnDTW(object):
    ...
    def _dist_matrix(self, x, y):
        ...
        blocks_per_grid = (x.shape[0] // threads_per_block[0] + 1,
                           y.shape[0] // threads_per_block[1] + 1)

        distance_matrix = np.empty((x.shape[0], y.shape[0]), dtype=np.float64)
        dm = dtw_distance[blocks_per_grid, threads_per_block](x, y, distance_matrix)
